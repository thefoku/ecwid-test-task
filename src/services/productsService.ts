...
export function useProducts() {
  ...
  async function fetchProductById(id: number): Promise<EcwidProduct | null> {
    const url = `https://app.ecwid.com/api/v3/${storeId}/products/${id}`
    try {
      const response = await fetch(url, {
        headers: {
          Authorization: `Bearer ${TOKEN}`,
